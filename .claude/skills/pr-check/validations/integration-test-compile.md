...
The common shape is an error naming a package that plainly exists in the tree, such as `package com.liferay.portal.kernel.model does not exist`. `install-portal-snapshots` installs poms whose dependency versions are unsubstituted build tokens, so `portal-impl` resolves carrying nothing and the kernel never reaches any `testIntegrationCompileClasspath`. Every `-test` module whose `testIntegration` sources import a kernel or petra package fails that way on every branch, which is nearly all of them.
