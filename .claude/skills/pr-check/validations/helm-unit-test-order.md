...
do
	descriptions=$(grep '^        it: ' "${file}" | sed 's/^        it: //')
...
Every `UNSORTED` line is a failure. Report the file and the descriptions that sit out of order, and say that sorting the `tests` entries is the fix.
