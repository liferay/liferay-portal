...
describe('FilterPicker', () => {
	...
	describe('paging on scroll', () => {
		...
		const getPageVariables = (page: number) => ({page, pageSize: 2});

		// jsdom reports every box as zero-sized, so the scroll geometry has to
		// be planted before the event is dispatched.
		...
		const openWithFirstPage = async (dataSourceFn: jest.Mock) => {
			renderPicker({
				dataSourceFn,
				getPageVariables,
				variables: VARIABLES,
		...
		it('requests the next page when the option list is scrolled to the bottom', async () => {
			const dataSourceFn = jest
				.fn()
				.mockResolvedValue({items: ITEMS, totalCount: 4});

			await openWithFirstPage(dataSourceFn);
			...
			dataSourceFn.mockResolvedValue({items: PAGE_ONE, totalCount: 4});

			scrollToBottom(screen.getByRole('listbox'));
		...
		it('appends the next page to the options already loaded', async () => {
			const dataSourceFn = jest
				.fn()
				.mockResolvedValue({items: ITEMS, totalCount: 4});
		...
		it('stops paging once every option has been loaded', async () => {
			const dataSourceFn = jest
				.fn()
				.mockResolvedValue({items: ITEMS, totalCount: 2});
		...
		it('shows the loading row at the end of the list while the next page loads', async () => {
			const dataSourceFn = jest
				.fn()
				.mockResolvedValue({items: ITEMS, totalCount: 4});
			...
			dataSourceFn.mockImplementation(
				() =>
					new Promise((resolve) => {
			...
			await waitFor(() => expect(dataSourceFn).toHaveBeenCalledTimes(2));

			expect(
				screen.getByRole('listbox').querySelector('.loading-root')
			).toBeInTheDocument();

			resolveNextPage({items: PAGE_ONE, totalCount: 4});
		...
		it('leaves the trigger caret alone while the next page loads', async () => {
			const dataSourceFn = jest
				.fn()
				.mockResolvedValue({items: ITEMS, totalCount: 4});
			...
			dataSourceFn.mockImplementation(() => new Promise(() => {}));

			scrollToBottom(screen.getByRole('listbox'));

			await waitFor(() => expect(dataSourceFn).toHaveBeenCalledTimes(2));

			// A spinner here would resize the trigger, and the menu is aligned
			// to it, so the whole dropdown would jump on every page.

			expect(getTrigger().querySelector('.loading-root')).toBeNull();
			expect(getTrigger()).toBeEnabled();
		});
