...
description: Commit a skill authored in this workspace to the liferay-portal repository, collapsing the workspace's duplicated agent trees into the repository's symlinked layout. Use when the user asks to commit, contribute, or upstream a skill, and as the handoff from `author-skill`.
name: commit-skill
...
The finish line is a **local commit on a fresh branch**. There is no push and no pull request — the user sends it for review themselves.
...
The `[:/]` prefix matches both `git@github.com:` and `https://github.com/` forms, and `(\.git)?$` anchors past an inconsistent suffix. Anchoring is what keeps forks under `liferay-devtools`, `liferay-one`, and personal accounts from matching.
...
Check three sources in order, and stop at the first that answers.

**Session context.** The common case is `author-skill` having just run, so the names are already known. Confirm them with the user rather than deriving them again.
...
**Baseline comparison.** When the workspace is not a repository, diff it against the portal repo's own tree, which has the same layout:
...
Work from a freshly pulled `master` so the baseline and the commit share a revision:
...
Each vendor names its entry point differently, so read the existing one in the repo rather than assuming a name. Match whatever vendor directories the repository already carries; adding a tree it does not have is a separate decision, not part of contributing a skill.

Skill Router and Preflight Rule edits collapse the same way. Two separately edited files in the workspace become one edit to `.agents/liferay-rules.md`, since each vendor entry point already points at it.
...
Its ticket resolution reads the current branch and recent commits, which on a freshly created branch describe whatever the user was last working on rather than this contribution. Resolve the ticket by situation instead:
...
- **A straight copy produces the wrong tree.** The workspace's vendor copies are real files; the repo expects symlinks. Copying them over commits duplicated content that the publish step already produces on its own.
...
- **The stash is the one step that looks destructive.** Report its ref every time. The user decides when to restore it.
