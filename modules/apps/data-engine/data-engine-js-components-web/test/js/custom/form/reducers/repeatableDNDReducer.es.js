...
describe('Repeatable Drag and Drop Reducer', () => {
	it('renumbers only the reordered instances when a nested repeatable field with sibling fields changes order (LPP-64814)', () => {
		const state = {
			pages: createPages([
				{
					fieldName: 'Question',
		...
		expect(answers.map(({nestedFields}) => nestedFields[0].name)).toEqual([
		...
		expect(answers.map(({nestedFields}) => nestedFields[0].value)).toEqual([
			'answer-two',
			'answer-one',
			'answer-three',
		]);
	...
	it('renumbers top-level repeatable field instances by their new order', () => {
		...
		expect(fields.map(({value}) => value)).toEqual(['three', 'one', 'two']);
	});
});
