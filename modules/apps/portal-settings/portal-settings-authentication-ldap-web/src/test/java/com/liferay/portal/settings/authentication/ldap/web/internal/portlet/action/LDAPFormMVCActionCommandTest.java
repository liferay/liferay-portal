...
public class LDAPFormMVCActionCommandTest {
	...
	public void testDoProcessAction() throws Exception {
		...
		_throwOnUpdateProperties(ldapConfigurationModelListenerException);
		...
		_throwOnUpdateProperties(new PrincipalException());

		MockActionRequest otherMockActionRequest = _getMockActionRequest();

		SessionErrors.clear(otherMockActionRequest);

		Assert.assertThrows(
			SystemException.class,
			() -> _ldapFormMVCActionCommand.doProcessAction(
				otherMockActionRequest, new MockActionResponse()));

		Assert.assertFalse(
			SessionErrors.contains(
				otherMockActionRequest,
				LDAPConfigurationModelListenerException.class.getName()));
	}
	...
	private void _throwOnUpdateProperties(Throwable throwable) {
		Mockito.doThrow(
