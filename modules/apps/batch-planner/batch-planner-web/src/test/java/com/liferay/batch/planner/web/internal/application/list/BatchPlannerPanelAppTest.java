 ...
 */
public class BatchPlannerPanelAppTest {
	...
	public void setUp() {
		ReflectionTestUtil.setFieldValue(
			_batchPlannerPanelApp, "_language", _language);
	...
	public void testGetPanelAppNavigationItems() throws Exception {
		...
		Assert.assertEquals(
			"templates", panelAppNavigationItem.getCanonicalName());
		Assert.assertEquals("templates-es", panelAppNavigationItem.getLabel());
		...
		Assert.assertTrue(
			href,
			href.contains(
				"mvcRenderCommandName=/batch_planner" +
					"/view_batch_planner_plan_templates"));
		Assert.assertTrue(
			href, href.contains("tabs1=batch-planner-plan-templates"));
	}
	...
	public void testGetPanelAppNavigationItemsOmitsTheDefaultViewCommand()
		...
		Assert.assertEquals(
			"import-and-export", panelAppNavigationItem.getCanonicalName());
		Assert.assertEquals(
			"import-and-export-es", panelAppNavigationItem.getLabel());

		String href = panelAppNavigationItem.getHref();

		Assert.assertTrue(href, href.contains("tabs1=batch-planner-plans"));
		Assert.assertFalse(href, href.contains("mvcRenderCommandName"));
	}
