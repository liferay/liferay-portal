 ...
 */
public class AnnouncementsPanelAppTest {
	...
	public void setUp() {
		ReflectionTestUtil.setFieldValue(
			_announcementsPanelApp, "_language", _language);
	...
	public void testGetPanelAppNavigationItems() throws Exception {
		List<PanelAppNavigationItem> panelAppNavigationItems =
			_announcementsPanelApp.getPanelAppNavigationItems(
				_httpServletRequest);
		...
		Assert.assertEquals(
			"announcements", panelAppNavigationItem.getCanonicalName());
		Assert.assertEquals(
			"announcements-es", panelAppNavigationItem.getLabel());

		String href = panelAppNavigationItem.getHref();

		Assert.assertTrue(href, href.contains("navigation=announcements"));

		panelAppNavigationItem = panelAppNavigationItems.get(1);

		Assert.assertEquals(
			"alerts", panelAppNavigationItem.getCanonicalName());
		Assert.assertEquals("alerts-es", panelAppNavigationItem.getLabel());

		href = panelAppNavigationItem.getHref();

		Assert.assertTrue(href, href.contains("navigation=alerts"));
	}

	private final AnnouncementsPanelApp _announcementsPanelApp =
		new AnnouncementsPanelApp() {
