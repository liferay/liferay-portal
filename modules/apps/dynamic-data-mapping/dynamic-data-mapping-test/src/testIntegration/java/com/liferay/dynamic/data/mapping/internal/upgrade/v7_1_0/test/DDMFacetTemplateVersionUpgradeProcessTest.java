	...
	public void testUpgrade() throws Exception {
		...
		try (Connection connection = DataAccess.getConnection();
			PreparedStatement preparedStatement = connection.prepareStatement(
				"update DDMTemplateVersion set classNameId = ? where " +
					"templateId = ?")) {

			preparedStatement.setLong(1, oldClassNameId);
			preparedStatement.setLong(2, templateId);

			preparedStatement.executeUpdate();
		}

		long persistedVersionClassNameIdBeforeUpdate;
		...
		try (Connection connection = DataAccess.getConnection();
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				...
				persistedVersionClassNameIdBeforeUpdate = resultSet.getLong(1);
			}
		}

		Assert.assertEquals(
			oldClassNameId, persistedVersionClassNameIdBeforeUpdate);

		upgradeProcess.upgrade();

		long persistedVersionClassNameIdAfterUpdate;
		...
		try (Connection connection = DataAccess.getConnection();
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				Assert.assertTrue(resultSet.next());
				persistedVersionClassNameIdAfterUpdate = resultSet.getLong(1);
			}
		}

		Assert.assertEquals(
			newClassNameId, persistedVersionClassNameIdAfterUpdate);
	}
