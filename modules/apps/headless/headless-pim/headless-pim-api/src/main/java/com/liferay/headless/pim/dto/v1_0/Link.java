...
@io.swagger.v3.oas.annotations.media.Schema(
	requiredProperties = {"sourceLinkReference", "type"}
)
...
public class Link implements Serializable {
	...
	public LinkReference getSourceLinkReference() {
		if (_sourceLinkReferenceSupplier != null) {
			sourceLinkReference = _sourceLinkReferenceSupplier.get();

			_sourceLinkReferenceSupplier = null;
		}
	...
	public void setSourceLinkReference(
		...
		_sourceLinkReferenceSupplier = () -> {
			try {
				return sourceLinkReferenceUnsafeSupplier.get();
			}
	...
	@JsonIgnore
	private Supplier<LinkReference> _sourceLinkReferenceSupplier;

	@io.swagger.v3.oas.annotations.media.Schema
	@Valid
	public LinkReference[] getTargetLinkReferences() {
		if (_targetLinkReferencesSupplier != null) {
			targetLinkReferences = _targetLinkReferencesSupplier.get();

			_targetLinkReferencesSupplier = null;
		}
	...
	public void setTargetLinkReferences(
		...
		_targetLinkReferencesSupplier = () -> {
			try {
				return targetLinkReferencesUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
	...
	@JsonIgnore
	private Supplier<LinkReference[]> _targetLinkReferencesSupplier;
	...
	public String toString() {
		...
		LinkReference sourceLinkReference = getSourceLinkReference();

		if (sourceLinkReference != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			sb.append(String.valueOf(sourceLinkReference));
		}

		LinkReference[] targetLinkReferences = getTargetLinkReferences();

		if (targetLinkReferences != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < targetLinkReferences.length; i++) {
				sb.append(String.valueOf(targetLinkReferences[i]));

				if ((i + 1) < targetLinkReferences.length) {
					sb.append(", ");
				}
			}
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		...
		defaultValue = "com.liferay.headless.pim.dto.v1_0.Link",
		name = "x-class-name"
	)
...
// LIFERAY-REST-BUILDER-HASH:-1802587016