...
public class DataMaskWriterInterceptorTest {
	...
	public void setUp() {
		ReflectionTestUtil.setFieldValue(
			_dataMaskWriterInterceptor, "_dataMaskEngine", _dataMaskEngine);
		ReflectionTestUtil.setFieldValue(
			_dataMaskWriterInterceptor, "_httpServletRequest",
			_httpServletRequest);
		ReflectionTestUtil.setFieldValue(
			_dataMaskWriterInterceptor, "_portal", _portal);
	}
	...
	public void testAroundWriteTo() throws Exception {
		...
		_dataMaskWriterInterceptor.aroundWriteTo(_writerInterceptorContext);
		...
		Mockito.verify(
			_dataMaskEngine, Mockito.never()
		).redact(
			Mockito.anyLong(), Mockito.anyList(), Mockito.anyString()
		);
	}

	private final DataMaskEngine _dataMaskEngine = Mockito.mock(
		DataMaskEngine.class);
	private final DataMaskWriterInterceptor _dataMaskWriterInterceptor =
		new DataMaskWriterInterceptor();
