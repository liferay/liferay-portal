...
Set the `ai` prop to give the menu the AI treatment — items highlight in purple
(`$purple-l5` background, `$purple` text) on hover, focus, and active. Use
`symbolLeft` for leading sparkle/action icons and `ClayDropDown.Group` for the
grouped headers. For the full stateful flow (menu → prompt → working → result),
use `ClayDropDownWithAI`, which composes the prompt composer and the
working/accept states with `ClayAIButton`.
