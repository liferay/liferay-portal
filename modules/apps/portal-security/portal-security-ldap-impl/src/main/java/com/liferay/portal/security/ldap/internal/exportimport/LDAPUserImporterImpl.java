...
public class LDAPUserImporterImpl implements LDAPUserImporter {
	...
	protected User addUser(long companyId, LDAPUser ldapUser, String password)
		...
		if (!ldapImportConfiguration.importUserPasswordEnabled()) {
			...
			if (!autoPassword) {
				String defaultPassword = _secretResolver.resolve(
					companyId,
	...
	private User _updateUser(
		...
		if ((modifiedDate != null) &&
			...
			if ((ldapUser.isUpdatePassword() ||
				...
				_updateUserPassword(
					user.getCompanyId(), ldapImportConfiguration,
					user.getUserId(), user.getScreenName(), password,
					passwordReset, modifiedDate);
		...
		if (ldapUser.isUpdatePassword() ||
			...
			password = _updateUserPassword(
				user.getCompanyId(), ldapImportConfiguration, user.getUserId(),
	...
	private String _updateUserPassword(
			long companyId, LDAPImportConfiguration ldapImportConfiguration,
			long userId, String screenName, String password,
			boolean passwordReset, Date modifiedDate)
		throws Exception {
		...
		if (!ldapImportConfiguration.importUserPasswordEnabled()) {
			...
			if (ldapImportConfiguration.importUserPasswordAutogenerated()) {
				...
				password = _secretResolver.resolve(
					companyId,
					ldapImportConfiguration.importUserPasswordDefault());
