...
public class KaleoDefinitionUpgradeProcess extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		_upgradeKaleoDefinition("kaleoDefinitionId", "KaleoDefinition");
	}

	private void _upgradeKaleoDefinition(String columnName, String tableName)
		throws Exception {

		try (PreparedStatement preparedStatement1 = connection.prepareStatement(
				SQLTransformer.transform(
					StringBundler.concat(
						"select ctCollectionId, ", columnName, ", name from ",
			...
			PreparedStatement preparedStatement2 =
				AutoBatchPreparedStatementUtil.concurrentAutoBatch(
					...
					StringBundler.concat(
						"update ", tableName,
						"set externalReferenceCode = ?, set name = ? where ",
						"ctCollectionId = ? and ",
						columnName, " = ?"));
			...
			while (resultSet.next()) {
				preparedStatement2.setLong(
					3, resultSet.getLong("kaleoDefinitionId"));
