...
public class DBMigrationImport {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			DBMigrationImportProcess dbMigrationImportProcess =
				new DBMigrationImportProcess(
			...
			dbMigrationImportProcess.run();

			try (PrintWriter printWriter = new PrintWriter(
					new File(
						commandLine.getOptionValue("path"),
						"db_migration_import_report.txt"))) {
				...
				printWriter.println(
					StringUtil.merge(
						new Object[] {
							...
							dbMigrationImportProcess.getDataSourceInfos()
						},
						StringPool.NEW_LINE));
			}
	...
	private static void _printHelpAndExit(Options options) {
		new HelpFormatter(
			...
			"Liferay Portal Tools Database Migration Importer. This tool is " +
				"a beta feature. It is experimental and not supported.",
			options
		);
