 ...
 */
public class SXPBlueprintAdminPanelAppTest {
	...
	public void setUp() {
		ReflectionTestUtil.setFieldValue(
			_sxpBlueprintAdminPanelApp, "_language", _language);
	...
	public void testGetPanelAppNavigationItems() throws Exception {
		...
		Assert.assertEquals(
			"blueprints", panelAppNavigationItem.getCanonicalName());
		Assert.assertEquals("blueprints-es", panelAppNavigationItem.getLabel());

		String href = panelAppNavigationItem.getHref();

		Assert.assertTrue(href, href.contains("tabs1=sxpBlueprints"));
		Assert.assertTrue(
			href,
			href.contains(
				"mvcRenderCommandName=/sxp_blueprint_admin" +
					"/view_sxp_blueprints"));

		panelAppNavigationItem = panelAppNavigationItems.get(1);

		Assert.assertEquals(
			"elements", panelAppNavigationItem.getCanonicalName());
		Assert.assertEquals("elements-es", panelAppNavigationItem.getLabel());

		href = panelAppNavigationItem.getHref();

		Assert.assertTrue(href, href.contains("tabs1=sxpElements"));
		Assert.assertTrue(
			href,
			href.contains(
				"mvcRenderCommandName=/sxp_blueprint_admin/view_sxp_elements"));
	}
