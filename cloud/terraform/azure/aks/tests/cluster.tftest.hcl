...
run "should_enable_monitor_metrics_when_observability_enabled" {
	assert {
		...
		error_message="Enabling observability must create the monitor_metrics block"
	}
	command=plan
	variables {
...
run "should_not_enable_monitor_metrics_by_default" {
	assert {
		...
		error_message="Observability must be disabled by default so no monitor_metrics block is created"
	}
	command=plan
}
